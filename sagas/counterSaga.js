...
function* increaseCounterAsync() {
  try {
    ...
    yield put({
      type: COUNTER.INCREASE_COUNTER_ASYNC,
      value: 1,
    });
  }
  catch (error) {
...
export function* watchIncreaseCounter() {
  ...
  yield takeLatest(COUNTER.INCREASE_COUNTER, increaseCounterAsync);
};
...
function* decreaseCounter() {
  try {
    // Dispatch Action To Redux Store
    yield put({
      type: COUNTER.DECREASE_COUNTER_ASYNC,
      value: 1,
    });
  }
  catch (error) {
...
export function* watchDecreaseCounter() {
  // Take Last Action Only
  yield takeLatest(COUNTER.DECREASE_COUNTER, decreaseCounter);
};